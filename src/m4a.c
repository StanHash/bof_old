...
#define SOUND_INFO_PTR (*(struct SoundInfo **) 0x03007FF0)
...
SECTION(".MPlayJumpTable")
MPlayFunc MPlayJumpTable[36] = { 0 };

SECTION(".CgbChans")
struct CgbChannel CgbChans[4] = { { 0 } };

SECTION(".MPlayMemAccArea")
u8 MPlayMemAccArea[0x10] = { 0 };

typedef void (* XcmdFunc)(struct MusicPlayer *, struct MusicPlayerTrack *);

extern XcmdFunc const XcmdTable[];

extern MPlayFunc const MPlayJumpTableTemplate[];
extern u8 const ScaleTable[];
extern u32 const FreqTable[];
extern u16 const PcmSamplesPerVBlankTable[];

extern u8 const CgbScaleTable[];
extern i16 const CgbFreqTable[];
extern u8 const NoiseTable[];

extern u8 const ScaleTable[];
extern u32 const FreqTable[];

extern const u8 Cgb3Vol[];
...
void SoundInit(struct SoundInfo * sound_info)
{
    ...
    if (REG_DMA1CNT & DMA_REPEAT)
        REG_DMA1CNT = (DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INCREMENT | DMA_DST_FIXED) | 4;

    if (REG_DMA2CNT & DMA_REPEAT)
        REG_DMA2CNT = (DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INCREMENT | DMA_DST_FIXED) | 4;

    REG_DMA1CNT_H = DMA_32BIT >> 16;
    REG_DMA2CNT_H = DMA_32BIT >> 16;
    REG_SOUNDCNT_X = SOUND_MASTER_ENABLE
        | SOUND_4_ON
        | SOUND_3_ON
        | SOUND_2_ON
...
void SampleFreqSet(u32 freq)
{
    ...
    sound_info->pcm_samples_per_vblank = PcmSamplesPerVBlankTable[freq - 1];
...
void m4aSoundVSyncOn(void)
{
    ...
    REG_DMA1CNT_H = (DMA_ENABLE | DMA_START_SPECIAL | DMA_32BIT | DMA_REPEAT) >> 16;
    REG_DMA2CNT_H = (DMA_ENABLE | DMA_START_SPECIAL | DMA_32BIT | DMA_REPEAT) >> 16;
...
u32 MidiKeyToCgbFreq(u8 chan_num, u8 key, u8 fine_adjust)
{
    ...
    else
    {
        ...
        val1 = CgbScaleTable[key];
        val1 = CgbFreqTable[val1 & 0xF] >> (val1 >> 4);

        val2 = CgbScaleTable[key + 1];
        val2 = CgbFreqTable[val2 & 0xF] >> (val2 >> 4);
...
void CgbSound(void)
{
    ...
    envelope_complete:
        ...
        if (channels->modify & CGB_CHANNEL_MO_VOL)
        {
            ...
            if (ch == 3)
            {
                *nrx2ptr = Cgb3Vol[channels->envelope_volume];
                if (channels->n4 & 0x80)
                {
                    *nrx0ptr = 0x80;
                    *nrx4ptr = channels->n4;
...
void ply_xcmd(struct MusicPlayer * music_player, struct MusicPlayerTrack * track)
{
    ...
    XcmdTable[n](music_player, track);
}
...
MPlayFunc const MPlayJumpTableTemplate[] =
{
    ply_fine,
    ply_goto,
    ply_patt,
...
u16 const PcmSamplesPerVBlankTable[] =
{
    96,
    132,
    176,
